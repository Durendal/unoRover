...
class core:
    ...
    def loadCore(self, core):
        ...
        if core == "core." or "__init__" in core or ".pyc" in core:
            return False
        coref = "core."+str(core)
        if not os.path.isfile("core"+self.fw.seperator+core+".py"):
    ...
    def preloadCore(self):
        ...
        core = glob.glob("core"+self.fw.seperator+"*.py")
        core_files = []
        for i in range(len(core)):
            print core[i][5:-3]
            self.loadCore(core[i][5:-3])
            print core[i].split(".")[0].split(""+self.fw.seperator)
            core[i] = core[i].split(".")[0].split(""+self.fw.seperator)[1]
            core[i] = "core."+core[i]
    ...
    def listCore(self):
        ...
        classes = glob.glob("core"+self.fw.seperator+"*.py")
